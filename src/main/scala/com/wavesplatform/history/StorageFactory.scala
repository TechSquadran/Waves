...
object StorageFactory {
  ...
  def apply(settings: WavesSettings, db: DB, time: Time): (NgHistory with DebugNgHistory, StateWriter with SnapshotStateReader, BlockchainUpdater) = {
    val stateWriter = new LevelDBWriter(db)
